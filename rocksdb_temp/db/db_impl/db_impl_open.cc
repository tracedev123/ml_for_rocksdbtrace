...
Status  DB::StartAllTrace(DB** dbptr) {
  ...
  if (op_trace_file != "") {
    ...
    fprintf(stdout, "Tracing the workload to11111 [%s]\n", op_trace_file.c_str());
  }
...
static Status StartDBImplAllTrace(DBImpl *impl) {
  ...
  rocksdb::TraceOptions block_cache_trace_options;
  rocksdb::TraceOptions io_trace_options;
  std::string op_trace_file  = "/tmp/op_trace_file_dbbench";
  std::string block_cache_trace_file = "/tmp/block_cache_trace_file_dbbench";
  std::string io_trace_file = "/tmp/io_trace_file_dbbench";
...
//    }
//
//    fprintf(stdout, "Tracing the workload to11wdwwdw11 [%s]\n", op_trace_file.c_str());
//  }
...
//
//  fprintf(stdout, "Total traces: [%lu]\n",
//          handler.GetCount());
...
Status DBImpl::Open(const DBOptions& db_options, const std::string& dbname,
  ...
  *dbptr = nullptr;
  ...
  impl->all_trace = true;
  fprintf(stdout, "ENDDDDfefefefFFEFD\n");
  return s;
}
}  // namespace ROCKSDB_NAMESPACE
