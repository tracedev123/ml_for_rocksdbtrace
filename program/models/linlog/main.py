...
for csv_file in all_csv_files_sorted:
    ...
    def find_highest_correlation(target_col, available_cols):
        correlations = correlation_matrix[target_col].loc[available_cols].abs()
        return correlations.idxmax(), correlations.max()
    columns_to_predict = set(columns_not_in_prediction)
    while columns_to_predict:
        highest_correlation = 0
        target_for_prediction = None
        predictor_col = None
        for col in columns_to_predict:
            current_predictor, current_correlation = find_highest_correlation(col, prediction_data.columns)
            if current_correlation > highest_correlation:
                highest_correlation = current_correlation
                target_for_prediction = col
                predictor_col = current_predictor
        num_rows = int(np.round(len(train_data) * 0.05))
        X_test = prediction_data[[predictor_col]]
        if target_for_prediction in continuous_columns:
            X_train = train_data[[predictor_col]].values[-num_rows:]
            y_train = train_data[target_for_prediction].values[-num_rows:]
            model = LinearRegression()
        else:
            X_train = train_data[[predictor_col]]
            y_train = train_data[target_for_prediction]
            model = LogisticRegression(max_iter=1000)
        model.fit(X_train, y_train)
        y_pred = model.predict(X_test)
        y_pred = np.where(y_pred < -1, -1, y_pred)
        if train_data[target_for_prediction].dtype == np.int64:
            y_pred = np.round(y_pred).astype(np.int64)
        prediction_data[target_for_prediction] = y_pred
        columns_to_predict.remove(target_for_prediction)
    columns_to_drop = ['index']
    data = data.drop(columns_to_drop, axis=1)
    train_data = train_data.drop(columns_to_drop, axis=1)
    test_data = test_data.drop(columns_to_drop, axis=1)
...
for csv_file in all_csv_files_sorted:
    ...
    X_train = train_data[[feature]].values[-num_rows:]
    X_test = prediction_data[[feature]]
    y_train = train_data[target].values[-num_rows:]
    model = LinearRegression()
    model.fit(X_train, y_train)
    y_pred = model.predict(X_test)
    y_pred = np.where(y_pred < -1, -1, y_pred)
    if train_data[target].dtype == np.int64:
        y_pred = np.round(y_pred).astype(np.int64)
    prediction_data[target] = y_pred
    columns_not_in_prediction.remove(target)
    feature = 'time'
    X_test = prediction_data[[feature]]
    for col in columns_not_in_prediction:
        if col in continuous_columns:
